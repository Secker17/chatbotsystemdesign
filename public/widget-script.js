(function() {
  ...
  function initGlassOrb(container, primaryColor) {
    ...
    container.style.boxShadow = '0 0 80px rgba(0,255,255,0.3), 0 0 120px rgba(0,200,255,0.2)';
    ...
    // Glass overlay - matching React GlassOrbAvatar
    const glass = document.createElement('div');
    glass.style.cssText = 'position:absolute;inset:0;border-radius:50%;pointer-events:none;z-index:1;overflow:hidden;' +
      'background:radial-gradient(circle at 30% 30%,rgba(255,255,255,0.2) 0%,rgba(0,255,255,0.1) 30%,rgba(0,200,255,0.05) 60%,rgba(0,100,200,0.1) 100%);' +
    ...
    // Idle palette matching React GlassOrbAvatar (cyan/teal)
    const palette = [
      { r: 0, g: 255, b: 255 },
      { r: 0, g: 240, b: 190 },
      { r: 50, g: 255, b: 255 },
  ...
  function initMiniGlassOrb(container, primaryColor, size) {
    container.innerHTML = '';
    container.style.background = 'transparent';
    container.style.boxShadow = '0 0 ' + Math.max(6, size * 0.4) + 'px rgba(0,255,255,0.3)';
  ...
  function createWidget() {
    ...
    async function startChat() {
      ...
      try {
        ...
        if (data.ai_greeting && data.ai_greeting !== (config && config.welcome_message)) {
          setTimeout(() => {
    ...
    function addMessage(content, type, isAi) {
      ...
      if (type === 'bot' || type === 'admin') {
        ...
        if (type === 'admin') {
          ...
          avatar.style.background = (config && config.primary_color) || '#14b8a6';
